...
import { Clock, Download, FileText, FolderOpen, ImageIcon, Film, Music, FileIcon, FileArchive, FileCode } from "lucide-react";
...
export const Route = createFileRoute("/dashboard/recent")({
  ...
  head: () => ({ meta: [{ title: "Recent — TeleVault" }] }),
});
...
function RecentPage() {
  ...
  // Filter items (not share links, not virtual folders, and not deleted)
  const recentItems = [...files]
    .filter((f) => f.mime_type !== "application/x-share-link" && f.mime_type !== "application/x-directory" && !f.filename.startsWith("trash/"))
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        eyebrow="Activity"
        title="Recent"
        description="Files you've touched or uploaded in the last 30 days."
      />
      ...
      <div className="mt-6">
        {filesQuery.isLoading ? (
          ...
          <EmptyState
            ...
            description="Once you start uploading files, they will appear here for quick access."
            action={
              <Button asChild className="rounded-xl">
                <Link to="/dashboard/files">Upload first file</Link>
              </Button>
            }
          />
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-border bg-surface/50 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="px-5 py-3.5 text-left">Upload Date</th>
                ...
                {recentItems.map((f) => {
                  ...
                  return (
                    <tr key={f.id} className="group transition-colors hover:bg-accent/40">
                      ...
                      <td className="px-5 py-3.5 text-muted-foreground">{formatDate(f.upload_date)}</td>
                      <td className="px-5 py-3.5">
